...
# <img src="public/tornedo_logo.svg" alt="tornedo - terminal torrent client" width="480">
...
Search multiple torrent sites **at once** into a single ranked, de-duplicated
result set, then download with the built-in WebTorrent engine. No account, no
cloud, no telemetry. State lives in a local SQLite database and survives
restarts.
...
**Watch online**: search and play movies, TV and live TV from the terminal
without downloading. All streaming providers ship enabled by default.
...
- **Live TV**: add any m3u8 playlist (`tornedo tv add <url>`) and matching
  ...
  their stream URL.
- **Download from Watch**: `d` downloads any stream with Range resume via
...
- **Federated search**: every enabled source is searched with its own timeout
  and fault isolation (up to 8 sources concurrently); a slow or dead source
  never blocks the rest. Results stream in as each source settles. Timeouts,
  outages and 5xx responses are retried once after 250ms; parse failures are
  never retried.
- **Smart results**: titles are parsed into structured metadata (quality,
  ...
  and version...). The same torrent found on different sites is merged by
  infohash, everything is deterministically ranked, and releases group by
  title/year/season with each quality as a variant.
- **Query analysis**: every query is parsed for media type, title,
  artist/album, year, quality, resolution, and season/episode. Releases that
  match the inferred signals get a ranking bonus (see `ranking.*`), sources
  ...
  reaches 0.5.
- **Downloads**: WebTorrent engine behind a thin abstraction, persistent queue,
  resume support, global speed limits, seeding.
...
- **Terminal UI**: a component-driven TUI built on **Ink + React**:
...
> project. Skipping the optional addons only removes WebRTC and µTP; DHT peer
> discovery still runs over UDP, trackers over HTTP/UDP/WebSocket, and peer
> transfer over plain BitTorrent TCP.
>
...
>
> ```sh
> npx --yes only-allow pnpm     # exits 1 with a "Use pnpm" box, which is expected
> npm install -g tornedo
> ```
>
...
All providers ship enabled: no addon, playlist, or config step is required
before watching:
...
- **Torznab/Newznab**: point at any local indexer (Prowlarr, Jackett, nZEDb,
  ...). Tornedo reads `?t=caps` and calls only the query modes the endpoint
...
silently empty) when their page structure changes.
